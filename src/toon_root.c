...
ToonGetRootWindow(Display *display, int screen, Window *clientparent)
{
  ...
  *clientparent = root;
  ...
  if (XGetWindowProperty(display, root,
    ...
    XFree((char *) toplevel);
    if (background) {
      strcpy(toon_message, "Drawing to Nautilus Desktop");
  ...
  if (!background
	...
	if (XGetWindowProperty(display, root,
	  ...
	  if (XGetWindowProperty(display, root,
	      ...
	      || !tmpwindow) {
	    /* Must be KDE 2.1+ */
	    strcpy(toon_message, "Drawing to KDE Desktop");
	    background = *newroot;
	  }
	  else if (tmpwindow) {
	...
	if (!background) {  
	  ...
	  strcpy(toon_message, "Drawing to virtual root window");
  ...
  if (!background) {
	...
	sprintf(toon_message, "Drawing to Enlightenment Desktop %lu (the root window)", *desktop);
	background = root;
	XFree((char *) desktop);
      }
	...
	for (i = 0; i < nChildren; ++i) {
	  if (XGetWindowProperty(display, children[i],
	    ...
	    sprintf(toon_message, "Drawing to Enlightenment Desktop %lu", *desktop);
